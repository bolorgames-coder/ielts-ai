...
const { buildPrompt, buildFeedbackPrompt, scoreFromObservations } = require("./rubric.js");
...
async function evaluateEssay({ task, question, visual, essay, feedback }) {
  ...
  if (feedback !== false) {
    const fb = await getFeedback(buildFeedbackPrompt, essay);
    if (fb) {
      result.corrections = fb.corrections || [];
      result.strengths = fb.strengths || [];
      result.weaknesses = fb.weaknesses || [];
    }
  }
