...
const { buildPrompt, buildFeedbackPrompt, scoreFromObservations, sanitizeCorrections, buildNotes } = require("./rubric.js");
...
async function evaluateEssay({ task, question, visual, essay, feedback }) {
  ...
  // Давуу тал / засах зүйлийг загвараас биш, кодоор бичнэ.
  // (Загварын монгол хэл найдваргүй, дүгнэлт нь оноотой зөрдөг)
  const notes = buildNotes(obs, taskNumber, {
    task: result.criteria.task.score,
    coherence: result.criteria.coherence.score,
    lexical: result.criteria.lexical.score,
    grammar: result.criteria.grammar.score,
  }, String(visual || "").trim().length > 10);
  result.strengths = notes.strengths;
  result.weaknesses = notes.weaknesses;
