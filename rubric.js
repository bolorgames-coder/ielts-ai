...
function buildFeedbackPrompt(essay) {
  const system =
    "You are an English teacher. You find the most costly errors in a student text and correct them. " +
    "You explain in Mongolian Cyrillic. You output strict JSON.";

  const user = `
Find the errors in the text below that cost the most marks in an IELTS Writing exam.
...
{
  "corrections": [
    { "original": "short phrase copied from the text", "corrected": "the fixed version", "type": "grammar", "explanation": "one short line in Mongolian Cyrillic" }
  ],
...
RULES:
- Give 5 to 8 corrections. Choose the errors that matter most, not every comma.
...
module.exports = { buildPrompt, buildFeedbackPrompt, scoreFromObservations };
