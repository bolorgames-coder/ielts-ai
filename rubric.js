...
// ---------- 1г. ДҮГНЭЛТИЙГ КОДООР БИЧИХ ----------
// Загварын монгол хэл найдваргүй тул давуу тал / засах зүйлийг
// ажиглалтын тоон үзүүлэлтээс код өөрөө бичнэ. Оноотой үргэлж нийцнэ.

function buildNotes(o, taskNumber, sc, hasVisual) {
  const n = (v, d = 0) => (typeof v === "number" && isFinite(v) ? v : d);
  const a = (v) => (Array.isArray(v) ? v : []);
  const sent = Math.max(1, n(o.sentenceCount, 1));
  const strengths = [];
  const weaknesses = [];
  ...
  if (taskNumber === 1) {
    if (o.overviewPresent === true && o.overviewSpecific === true)
      strengths.push("Ерөнхий дүгнэлт (Overview) тодорхой бөгөөд гол хандлагыг нэрлэсэн — Task 1-д хамгийн их оноо авдаг хэсэг.");
    const cov = n(o.keyFeaturesCovered) / Math.max(1, n(o.keyFeaturesRequired, 1));
    if (cov >= 0.8) strengths.push("Зураг дээрх гол өөрчлөлтүүдийг бараг бүгдийг нь хамарсан.");
    if (o.dataUsed === true) strengths.push("Тодорхой тоо, нэршлийг эх сурвалжаас зөв иш татсан.");
  } else {
    if (o.positionClear === true) strengths.push("Өөрийн байр суурийг эхнээс нь тодорхой илэрхийлж, төгсгөл хүртэл тууштай барьсан.");
    if (n(o.mainIdeas) >= 3) strengths.push(`Үндсэн санаа ${n(o.mainIdeas)} ширхэг хөгжүүлсэн нь агуулгын хувьд баялаг.`);
    if (n(o.specificExamples) >= 2) strengths.push("Хийсвэр ярианд автахгүй, бодит жишээгээр баталсан.");
  }
  if (n(o.paragraphCount) >= 4 && o.logicalOrder === true)
    strengths.push("Догол мөрийн хуваарилалт зөв, бодлын урсгал логиктой.");
  const soph = a(o.sophisticatedLexis).length;
  if (soph >= 5) strengths.push("Өндөр түвшний үг хэллэг (academic collocation) элбэг ашигласан.");
  else if (soph >= 2) strengths.push("Зарим өндөр түвшний хэллэг зөв байранд нь хэрэглэсэн.");
  const clean = n(o.errorFreeSentences) / sent;
  if (clean >= 0.5) strengths.push(`Өгүүлбэрийн ${Math.round(clean * 100)}% нь ямар ч дүрмийн алдаагүй.`);
  if (n(o.complexSentences) / sent >= 0.4) strengths.push("Нийлмэл өгүүлбэр элбэг — дүрмийн хүрээ өргөн.");

  // ── Засах зүйл: хамгийн бага оноотой шалгуурт чиглэнэ ──
  const low = Object.entries(sc).sort((x, y) => x[1] - y[1])[0];
  const lowName = { task: "Task Achievement", coherence: "Coherence & Cohesion",
                    lexical: "Lexical Resource", grammar: "Grammatical Range" }[low[0]];

  if (low[0] === "task" && taskNumber === 1) {
    if (o.overviewPresent !== true) weaknesses.push("Overview дутуу. «Overall, ...» гэж эхэлсэн 1-2 өгүүлбэрээр ерөнхий хандлагыг нэгтгэж бич — энэ нь Task 1-д хамгийн их оноо алдуулдаг цэг.");
    else if (o.overviewSpecific !== true) weaknesses.push("Overview байгаа ч ерөнхий байна. «Юу хамгийн их байсан», «аль нь хамгийн их өөрчлөгдсөн» гэдгийг нэрлэж бич.");
    else weaknesses.push("Гол өөрчлөлтүүдийг бүлэглэж харьцуулбал илүү өндөр оноо авна. Он тус бүрээр биш, хандлагаар нь нэгтгэ.");
    // Зургийн тайлбар бөглөөгүй бол ажиглагч зураг дээр юу байгааг мэдэхгүй.
    // Тэр үед "дутуу", "таамагласан" гэсэн дүгнэлт зохиомол болох тул гаргахгүй.
    if (hasVisual && a(o.featuresMissing).length)
      weaknesses.push(`Дурдагдаагүй үлдсэн: ${a(o.featuresMissing).slice(0, 3).join(", ")}.`);
    if (hasVisual && o.speculation === true)
      weaknesses.push("Зураг дээр байхгүй шалтгааныг таамагласан. Task 1-д зөвхөн харагдаж буйг тайлбарлана.");
  } else if (low[0] === "task") {
    if (o.positionClear !== true) weaknesses.push("Байр суурь тодорхойгүй. Оршилд «I believe…» гэж шууд хэлээд, төгсгөлд давтан баталгаажуул.");
    else if (n(o.mainIdeas) < 4) weaknesses.push("Үндсэн санаа цөөн. Догол мөр тус бүрд нэг санааг гүн хөгжүүлж, 4-5 санаанд хүргэ.");
    if (n(o.specificExamples) < 2) weaknesses.push("Бодит жишээ дутуу. Нэрлэсэн улс, байгууллага, тодорхой нөхцөл байдал нэмбэл оноо өснө.");
    if (n(o.irrelevantParts) >= 2) weaknesses.push("Сэдвээс гажсан хэсэг байна. Догол мөр бүр асуултад шууд хариулж байгаа эсэхийг шалга.");
  }

  if (low[0] === "lexical" || soph < 3) {
    weaknesses.push(soph === 0
      ? "Үгийн сан энгийн байна. «of paramount importance», «a crucial part of», «by far the largest» зэрэг академик хэллэг нэмбэл Lexical оноо шууд өснө."
      : "Өндөр түвшний хэллэг цөөн. Ердийн үгийг илүү нарийн үгээр солих боломжийг хай (big → substantial, show → illustrate).");
  }
  if (a(o.spellingErrors).length >= 3)
    weaknesses.push(`Зөв бичгийн алдаа ${a(o.spellingErrors).length} ширхэг: ${a(o.spellingErrors).slice(0, 4).join(", ")}.`);

  if (low[0] === "grammar" || clean < 0.35) {
    if (n(o.complexSentences) / sent < 0.25) weaknesses.push("Нийлмэл өгүүлбэр дутуу. «which», «although», «while» ашиглан энгийн өгүүлбэрүүдийг холбож үз.");
    else weaknesses.push(`Алдаагүй өгүүлбэрийн эзлэх хувь ${Math.round(clean * 100)}% байна. 50%-аас дээш болговол Grammar 8.0 руу дөхнө.`);
  }
  if (low[0] === "coherence") {
    if (n(o.paragraphCount) < 4) weaknesses.push("Догол мөр цөөн. Санаа тус бүрийг тусад нь салгавал бүтэц тодорхой болно.");
    if (o.referencingClear === false) weaknesses.push("«this», «these», «they» зэрэг үг юуг зааж буй нь зарим газар бүрхэг байна.");
    if (n(o.linkerAtSentenceStart) / sent > 0.6) weaknesses.push("Өгүүлбэр болгон холбоос үгээр эхэлж байгаа нь механик сонсогдоно. Заримыг өгүүлбэрийн дунд шилжүүл.");
  }

  // Дор хаяж хоёр зүйл байлгана. Бүх оноо өндөр байсан ч дараагийн
  // band руу гарах тодорхой зөвлөгөө өгнө — "дутагдал алга" гэж орхихгүй.
  const NEXT = {
    coherence: "Coherence-ийг өсгөхийн тулд холбоос үгэнд бус, санааны урсгалд тулгуур. Догол мөр бүрийн эхний өгүүлбэр өмнөхтэйгөө агуулгаараа холбогдож байвал 8.0 руу дөхнө.",
    lexical: "Lexical 8.0-д хүрэхэд ховор бус, харин ЯГ ТААРСАН үг хэрэгтэй. Нэг санааг хоёр өөр өнгөөр илэрхийлэх боломжийг хай.",
    grammar: "Grammar 8.0-д өгүүлбэрийн бүтцийн олон янз байдал шаардлагатай. Inversion, cleft (It is X that…), participle clause зэргийг нэг нэгээр туршиж үз.",
    task: taskNumber === 1
      ? "Task 8.0-д хүрэхэд гол өгөгдлүүдийг зөвхөн нэрлэхгүй, хоорондоо харьцуулан тайлбарлах нь чухал."
      : "Task 8.0-д хүрэхэд санаа бүрийг эсрэг талын үзэл бодолтой харьцуулж, яагаад өөрийн байр суурь илүү хүчтэйг харуул.",
  };
  if (weaknesses.length < 2) weaknesses.push(NEXT[low[0]]);
  if (!weaknesses.length) weaknesses.push(`${lowName} (${low[1].toFixed(1)}) нь хамгийн бага үзүүлэлт тул энэ чиглэлд анхаар.`);
  if (!strengths.length) strengths.push("Даалгаврыг ойлгож, бүтэцтэй хариулахыг зорьсон нь харагдаж байна.");
...
module.exports = { buildPrompt, buildFeedbackPrompt, scoreFromObservations, sanitizeCorrections, buildNotes };
